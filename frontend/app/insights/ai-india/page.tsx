...
const stats = [
  ["68%", "of firms have AI pilots running"],
  ["11%", "have reached production scale"],
  ["3.2X", "ROI for mature adopters"],
  ["90", "days to first measurable value"],
];

const chapters = [
  ["01", "The Adoption Gap", "Why most Indian enterprises are stuck between AI pilots and real production deployment."],
  ["02", "Where AI Pays Off", "The functions and use cases delivering measurable returns today — and the ones that don't."],
  ["03", "The Operating Model", "How leading adopters structure teams, data, and governance to ship AI that lasts."],
  ["04", "Build, Buy, or Blend", "A decision framework for choosing your AI stack with unit economics, not hype."],
  ["05", "Risk & Responsibility", "Practical guardrails for compliance, security, and trustworthy AI at scale."],
  ["06", "The 12-Month Roadmap", "A staged plan to move from experimentation to an AI-native operating advantage."],
];
  ...
  return (
    <main style={FONT} className="w-full overflow-x-hidden bg-[#f3f3f1] text-[#0A1633]">
      <section className="relative overflow-hidden bg-[#0A1633] text-white">
        <div className="absolute -right-40 -top-40 h-[520px] w-[520px] bg-[#2563EB]/30 blur-[160px]" />
        <div className="relative mx-auto grid w-full max-w-[1220px] grid-cols-1 lg:grid-cols-[1fr_0.8fr] gap-12 px-5 sm:px-6 lg:px-8 pt-[80px] sm:pt-[104px] pb-[72px] items-center">
          <div>
            ...
            <motion.h1 {...rise()} className="text-[44px] sm:text-[62px] lg:text-[76px] font-medium leading-[0.92] tracking-[-0.045em]">
              AI for{" "}
            ...
            <motion.p {...rise(1)} className="mt-7 max-w-[52ch] text-[15px] sm:text-[17px] leading-[1.7] text-white/75">
              How Indian enterprises are moving AI beyond pilots into measurable
              revenue — the adoption gap, what pays off, and the roadmap to scale.
            </motion.p>
            ...
            <motion.div {...rise(2)} className="mt-9 flex flex-wrap items-center gap-4">
              ...
              <span className="text-[12px] uppercase tracking-[1.5px] text-white/55">Feb 2026 · PDF · 86 pages</span>
            </motion.div>
          </div>
          ...
          <motion.div {...rise(2)} className="relative mx-auto w-full max-w-[360px]">
            <div className="relative aspect-[3/4] overflow-hidden border border-white/15 shadow-2xl">
              <Image src="https://images.unsplash.com/photo-1620712943543-bcc4688e7485?q=80&w=1000&auto=format&fit=crop" alt="AI for Business India cover" fill className="object-cover opacity-90" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#0A1633]/85 to-transparent" />
              <div className="absolute bottom-0 left-0 p-7">
                <p className="text-[10px] uppercase tracking-[2px] text-white/60">NineXGroup Research</p>
                <p className="mt-2 text-[30px] font-medium leading-[1.02] tracking-[-0.03em]">AI for Business</p>
                <p className="text-[34px] font-medium leading-[0.9] tracking-[-0.04em] text-[#7db5ff]">India</p>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-[1px] border-x border-b border-[#0A1633]/10 bg-[#0A1633]/10">
          {stats.map((s, i) => (
            <motion.div key={s[0]} {...rise(i)} className="bg-[#f7f6f4] px-6 py-9 text-center sm:text-left">
              <div className="text-[44px] sm:text-[56px] font-medium leading-[0.9] tracking-[-0.04em] bg-gradient-to-r from-[#0A1633] to-[#2563EB] bg-clip-text text-transparent">{s[0]}</div>
      ...
      <section className="mx-auto w-full max-w-[1220px] px-5 sm:px-6 lg:px-8 pt-[80px] pb-[100px]">
        <motion.h2 {...rise()} className="max-w-[16ch] text-[32px] sm:text-[44px] font-medium leading-[1.02] tracking-[-0.035em]">
          What&lsquo;s inside the report
        </motion.h2>
        ...
        <motion.div {...rise()} className="mt-14 flex flex-col items-start justify-between gap-6 border border-[#0A1633]/10 bg-white p-8 sm:flex-row sm:items-center sm:p-10">
          <div>
            <h3 className="text-[24px] sm:text-[30px] font-medium tracking-[-0.03em]">Read it before your competitors do.</h3>
